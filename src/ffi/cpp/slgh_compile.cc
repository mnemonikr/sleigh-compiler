...
{
  ostringstream errs;
  errs << filename.back() << ":" << lineno.back() << " - ERROR " << msg;
  slgh_errors->push_back(errs.str());
  errors += 1;
  if (errors > 1000000) {
	throw SleighError("Too many errors: Aborting");
  }
}
...

{
  ostringstream warns;
  warns << "WARN  " << msg;
  slgh_warnings->push_back(warns.str());
}
...

{
  ...
  if (!slgh_warnings) {
      slgh_warnings = std::make_unique<vector<string>>();
  } else {
      slgh_warnings->clear();
  }
  ...
  try {
    ...
    if (sleighin == (FILE *)0) {
      ostringstream errs;
      errs << "Unable to open specfile: " << filein;
